...
function gradeAt(x, y) {
  ...
  return { stem: Math.floor((x - m.axisX) / m.cell), filtration: Math.max(0, Math.floor((m.axisY - y) / m.cell)) };
}
...
function packedClassInstances(ws, bounds, metrics) {
  ...
  return window.HFPSSCellLayout.packInstances(instances, metrics.cell, { baseYOffset: 0.16 });
}
...
function renderChart() {
  ...
  let markup = '<defs><marker id="arrow" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z" fill="#1f2937" /></marker></defs>';
  ...
  const packedInstances = packedClassInstances(ws, buffered, m);
  ...
  for (const relation of visibleRelations(ws, liveIds)) {
    ...
    markup += `<line class="relation-line ${relationVisualState(relation)}" data-relation="${escapeHtml(relation.id)}" x1="${from.x}" y1="${from.y}" x2="${to.x}" y2="${to.y}"><title>${escapeHtml(relation.statement)}</title></line>`;
  }
  for (const item of periodicDifferentials(ws, buffered)) {
    ...
    markup += `<line class="differential ${item.periodic ? "periodic" : ""} ${differentialVisualState(item.diff)}" x1="${from.x}" y1="${from.y}" x2="${to.x}" y2="${to.y}"/>`;
  }
  ...
  for (const record of packedInstances) {
    ...
    const classes = `${visualStateFor(ws, record.item)} ${selected} ${record.periodic ? "periodic" : ""}`;
    const label = classLabelMarkup(record, point, m, visible);
    const periodicAttribute = record.periodic ? ' data-periodic-copy="true"' : "";
    const aria = `${record.item.label} at ${gradeText(record.grade)}${record.periodic ? ", periodic copy" : ""}`;
...
function bindEvents() {
  ...
  chart.addEventListener("click", (event) => {
    ...
    const rect = chart.getBoundingClientRect();
    const fractional = gradeFloatAt(event.clientX - rect.left, event.clientY - rect.top);
    const m = chartMetrics();
    if (fractional.filtration < 0) return;
    const grade = gradeAt(event.clientX - rect.left, event.clientY - rect.top);
