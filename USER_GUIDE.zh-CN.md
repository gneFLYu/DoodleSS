...
periodicity 批量生成。Studio 不会把这些 legacy 操作自动解释为带来源的数学
...
- 周期复制在 Select 模式下打开其基础代数元的检查器。目前不能直接修改
  translated copy。
...
## D^8 周期性操作（已验证的单一自动分支）

此操作只适用于 integer-graded Q8 HFPSS 的 **E3 及以后**页面。选择图上的
class 后，在左侧始终可见的 **D8 periodicity** 区域操作。该区域即使当前
...
`E2`、`g=kD^3`、其他 workspace、under-review differential 和组合周期规则均为
**manual-only**：请使用 Generator/Differential/Relation 工具并附加审阅所需的
source locator 与 hypotheses；目前不会自动传播它们。
